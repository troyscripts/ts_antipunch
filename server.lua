...
local function log(message)
    print(('^3[TroyScripts]^7 [ts_antipunch] %s'):format(message))
end
...
CreateThread(function()
    ...
    log(('Versie %s gestart.'):format(current or 'onbekend'))
    ...
    PerformHttpRequest(url, function(status, body)
        ...
        if not latest then
            log('Updatecontrole: version.txt bevat geen geldige stabiele versie (bijvoorbeeld 1.8.1).')
            return
        end
        local comparison = 0
        for i = 1, 3 do
